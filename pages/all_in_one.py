...
def save_conversation_to_excel(client_number, messages):
    ...
    df.to_excel(
        f"data/output/client_{client_number}/conversation_client_{client_number}.xlsx",
        index=False
    )
# Streamlit UI
...
def main():
    ...
    if st.sidebar.button("Generate Profile and History", key="generate_profile_history_button"):
        if not client_exists(st.session_state.client_number) or st.session_state.agent_and_memory is None:
    ...
    if st.button("End/Save Conversation", key="end_save_conversation_button"):
        if st.session_state.client_number is not None and st.session_state.agent_and_memory is not None:
            ...
            save_conversation_to_excel(
                st.session_state.client_number, memory.chat_memory.messages)
            st.success(
